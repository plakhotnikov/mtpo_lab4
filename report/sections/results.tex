...
\midrule
\texttt{orders} & id (UUID PK), description, amount, status & UNIQUE(description, amount) & Хранение заказов \\
\texttt{payments} & id (UUID PK), order\_id (FK), amount, status & FK $\to$ orders(id) & Хранение платежей \\
\texttt{products} & id (BIGSERIAL PK), sku, name, price, version & UNIQUE(sku) & Каталог товаров \\
\texttt{idempotency\_keys} & id, idempotency\_key, method, path, response\_status, response\_body & UNIQUE(idempotency\_key) & Хранилище ключей \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}
    \item \textbf{IdempotencyFilter} --- HTTP-фильтр (\texttt{OncePerRequestFilter}), перехватывающий POST-запросы к \texttt{/api/orders} и \texttt{/api/payments}. Реализует алгоритм~\ref{alg:idempotency-check}. Может быть отключён через конфигурацию \texttt{app.idempotency.enabled=false}.
...
\begin{itemize}
    \item \textbf{Профиль по умолчанию} (\texttt{application.yml}): идемпотентность включена (\texttt{app.idempotency.enabled=true}), Flyway применяет миграции V1 и V2, все constraints активны.

    \item \textbf{Профиль non-idempotent} (\texttt{application-non-idempotent.yml}): идемпотентность отключена (\texttt{app.idempotency.enabled=false}), Flyway дополнительно применяет миграцию V3, которая удаляет UNIQUE constraints на \texttt{orders(description, amount)} и \texttt{products(sku)}.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{POST с одинаковым Idempotency-Key дважды} --- создаётся только один заказ, второй запрос возвращает кэшированный ответ. Проверяется через \texttt{assertEquals(1, orderRepository.count())}.
...
\begin{enumerate}
    ...
    \item \textbf{PUT с корректной версией} --- успешное обновление, \texttt{@Version} автоматически инкрементируется. Проверяется: \texttt{assertEquals(version + 1, updated.getVersion())}.
...
\begin{enumerate}
    \item \textbf{Импорт заказов из JSON} --- загрузка данных из файла \texttt{orders-input.json}, создание через API, сравнение количества, статусов и сумм с эталонным файлом \texttt{expected-orders-output.json}.
    \item \textbf{Импорт товаров из CSV} --- загрузка данных из файла \texttt{products-input.csv} через Jackson CSV, сохранение в БД, затем повторная попытка импорта тех же данных: все 3 попытки отклоняются UNIQUE constraint, количество записей не увеличивается.
\end{enumerate}
